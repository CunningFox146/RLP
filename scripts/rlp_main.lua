...
KnownModIndex.GetIsSpecialEventModWarningDisabled = function() return false end
...
if TheNet:GetServerGameMode() == "lavaarena" then
	...
	AddPrefabPostInit("lavaarena_boarlord", function(inst)
		...
		local function OnTalk(inst, data)
			_ontalkfn(inst, data)
			if data and data.message and inst.speechroot and pugna_sayings[data.message] then
...
env.AddGlobalClassPostConstruct("frontend", "FrontEnd", function(self)
	if POUpdater:IsDisabled() or InGamePlay() then
		return
	end
	
	if not self.updt_str then
		self.updt_str = AddUpdtStr(self.overlayroot)
	end
	
	self.updt_str:SetString("Проверка версии перевода...")
	POUpdater:ShouldUpdate(function(val)
		if val then
			self.updt_str:SetString("Обновление перевода...")
			
			local function OnUpdateDone(_, data)
				self.updt_str.inst:RemoveEventCallback("rlp_updated", OnUpdateDone, TheGlobalInstance)
				
				self.updt_str:SetString(data and "Перевод обновлен успешно." or "Произошла ошибка при обновлении.")
				self.updt_str.inst:DoTaskInTime(1, function()
					self.updt_str:Kill()
				end)
			end
			
			self.updt_str.inst:ListenForEvent("rlp_updated", OnUpdateDone, TheGlobalInstance)
			POUpdater:StartUpdating(true)
		else
			self.updt_str:SetString("Перевод последней версии.")
			self.updt_str.inst:DoTaskInTime(1, function()
				self.updt_str:Kill()
				self.updt_str = nil
			end)
