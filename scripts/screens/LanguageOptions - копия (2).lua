...
local LanguageOptions = Class(Screen, function(self)
	...
	self.UpdateNowButton = self.root:AddChild(ImageButton("images/ui.xml", "button_large.tex", "button_large_over.tex", "button_large_over.tex"))
...
--	self.HistoryButton:SetText("Вернуть лог")
    ...
    self.updateoptionspanel:SetPosition(left_col,105,0)
	...
	self.updateoptionstitle:SetScale(1.4, 1.4, 1)
	--self.updateoptionstitle:SetPosition(0, 240, 1)
    ...
    self.updateoptionstitle.text:SetRegionSize( 400, 70 )
	self.updateoptionstitle.text:SetPosition(0, 7.5, 1)
	...
	self.updateoptionstitle.modicon:SetScale(.5, .5, 1)
	...
	--Красивая шту4ка
	self.updateoptionstitle:MoveTo(Vector3(0, 350, 1), Vector3(0, 240, 1), .5, function()
		self.updateoptionstitle.modicon:Show()
		self.updateoptionstitle.modicon:MoveTo(Vector3(-275, -102, 1), Vector3(0, -102, 1), 1, function()
	...
	if not period then --Если нет записи о периоде, то делаем по умолчанию раз в неделю
		period=t.UpdatePeriod and t.UpdatePeriod[3] or "OncePerWeek"
	...
	for i,v in ipairs(t.UpdatePeriod) do
		if v==period then
	...
	self.updateoptionsspinner.OnChanged =
		function( _, data )
			...
			Profile:SetLocalizaitonValue("update_frequency",t.UpdatePeriod[self.updateoptionsspinner:GetSelectedIndex()])
		end
	...
	self.TranslationTypeText = self.updateoptionspanel:AddChild(Text(UIFONT, 40))
	self.TranslationTypeText:SetHAlign(ANCHOR_LEFT)
	self.TranslationTypeText:SetRegionSize( 320, 70 )
	self.TranslationTypeText:SetPosition(7, -85, 0)
	self.TranslationTypeText:SetString("Варианты русификации")
	self.TranslationTypeText:SetHAlign(ANCHOR_MIDDLE)
	...
	self.SpinnerBG:SetPosition(0,-165,0)
	--Сам спинер
	...
	self.TranslationTypeSpinner:SetPosition(0, -165, 0)
	...
	self.TranslationTypeDescriptionText = self.updateoptionspanel:AddChild(Text(BODYTEXTFONT, 25))
	...
	self.TranslationTypeDescriptionText:SetPosition(7, -420, 0)
	...
	self.URLButton:SetPosition(left_col-100, -280-buttonshift, 0)
	self.backbutton:SetPosition(left_col+100, -280-buttonshift, 0)
	...
	self.inst:DoTaskInTime(.75, function()
		self.eyebutton:Show()
		self.eyebutton:MoveTo(Vector3(250,0,1), Vector3(385,250,1), .75)
		
		self.inst:DoTaskInTime(.25, function()
			self.groupbutton:Show()
			self.groupbutton:MoveTo(Vector3(250,0,1), Vector3(385,100,1), .75)
			
			self.inst:DoTaskInTime(.25, function()
				self.vkbutton:Show()
				self.vkbutton:MoveTo(Vector3(250,0,1), Vector3(385,-105,1), .75)
				
				self.inst:DoTaskInTime(.25, function()
					self.wikibutton:Show()
					self.wikibutton:MoveTo(Vector3(250,0,1), Vector3(400,-260,1), .75)
				end)
			end)
		end)
	end)
...
function LanguageOptions:Cancel()
	...
	TheFrontEnd:FadeToScreen(TheFrontEnd:GetActiveScreen(), function() return MMScreen() end, nil)
end
