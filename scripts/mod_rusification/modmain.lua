mods.RusMods = {modinfo=modinfo}

--Проверяем наличие русификатора.
rus=mods.RussianLanguagePack or {}
mk=rus.RegisterRussianName

if not mk then --Без русификатора вообще нет смысла что-либо предпринимать.
	print("WARNING RUS MODS: Function \"RegisterRussianName\" not found.")
	return
end

--Фикс на несовершенство русификатора. В режиме "chat only" не должно быть краша.
if rus.CurrentTranslationType ~= nil and rus.TranslationTypes ~= nil and rus.TranslationTypes.Full ~= nil
	and rus.CurrentTranslationType ~= rus.TranslationTypes.Full --т.е. либо InterfaceChat, либо ChatOnly
then
	print("WARNING RUS MODS: RLP in \"Chat Only\" mode.")
	return
end

if _G.TheNet:IsDedicated() then --Нет смысла переводить что-либо на выделенном сервере.
	print("RUS MODS: Didicated server. Skipping fonts...")
	return
end
...
do
	local function SearchForModsByName()
		if not (_G.KnownModIndex and _G.KnownModIndex.savedata and _G.KnownModIndex.savedata.known_mods) then
			print("ERROR RUS MODS: Can't find KnownModIndex!")
			return
		end
...
--Функция формирования строк времени и прочих числительных
local st_tbl = {
	["дней"] = {"день","дня","дней"}, --1 день, 2 дня, 5 дней
...
function StringTime(n,s)
	...
	if days then
		...
		return n.." "..s --просто "N дней"
	end
end


--На этой стадии переводим настройки. Заодно
if not in_game then --Только если вне игры.
	print("RUS MODS: importing settings translation.")
	modimport("rus_settings.lua")
end
...
if not (in_game) then
	...
	--Клиент или хост, но не выделенный сервер (ибо нет смысла)
	print("RUS MODS: Not in game. Exit.")
	return
end
--print('RusMods activated!')

--Позволяет вводить русские символы при входе
--Фикс больше не нужен.
--
--AddClassPostConstruct("screens/lobbyscreen", function(self)
--	local NewValidChars=[[ abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,:;[]<>\@!#$%&()'*+-/=?^_{|}~"абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯІіЇїЄєҐґ]]
--	self.chatbox.textbox:SetCharacterFilter( NewValidChars )
--end)
--

local new_phrases = {nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,}
mods.RusMods.phrases = new_phrases
--Замена реплик персонажей
local old_TranslateToRussian = _G.rawget(_G,"TranslateToRussian") or rus.TranslateToRussian
if old_TranslateToRussian then
	rus.TranslateToRussian = function(message,entity,...)
		local new_line = string.find(message,"\n",1,true)
		--print("------ new_line="..tostring(new_line))
		--print("message:"..tostring(message))
		if new_line ~= nil then
			local mess1 = message:sub(1,new_line-1) --до конца строки
			--print("mess1("..#mess1.."):"..mess1)
			if new_phrases[mess1] then
				local mess2 = message:sub(new_line) --начиная с \n включительно и до конца строки
				--print("mess2:"..mess2)
	...
	_G.rawset(_G,"TranslateToRussian",rus.TranslateToRussian) --Совместимость с более старым русификатором.
else
	print("ERROR RUS MODS: Function \"TranslateToRussian\" not found.")
end
...
mods.RusMods.pp = pp --Экспортируем API функцию.
mods.RusMods.RegisterPhrase = pp


---Переименовываем действия по названию (id)
function RenameAction(act,new_name)
	if _G.rawget(_G,"ACTIONS") and _G.ACTIONS[act] then
		_G.ACTIONS[act].str = new_name
		if s.ACTIONS then
			s.ACTIONS[act] = new_name
		end
	end
end
mods.RusMods.RenameAction = RenameAction --Экспортируем API функцию.
--Далее небольшой фокус. Заменяем функцию русификатора, потому что она багнутая:
rus.RenameAction = RenameAction

--Регистрирует перевод рецептов на основе таблицы.
function RegisterRecipes(arr)
	for k,v in pairs(arr) do
		rec[k] = v --Тупо переводим строки построчно в реальный массив.
	end
end
mods.RusMods.RegisterRecipes = RegisterRecipes
...
-----------Временные фиксы перевода-----------
--STRINGS.UI.SERVERLISTINGSCREEN.FRIEND_ICON_HOVER = "Друзья уже играют здесь"
...
-------------------------------------------------------------------------------------------------------------
---------------------------------Теперь беремся за перевод модов.--------------------------------------------
-------------------------------------------------------------------------------------------------------------
...
if FindName("Beefalo Milk") then
	RegisterReplacedTranslation(function() --с подменой!!!
		modimport "mods/beefalo_milk.lua"
	end)
end	
...
if FindNameCut("Archery Mod") then
	RegisterTranslation(function() --без подмены
		modimport "mods/archery_mod.lua"
	end)
end
