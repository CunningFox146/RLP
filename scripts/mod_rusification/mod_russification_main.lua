print("Mod Translation: Checking mod translation...")

rus = mods.RussianLanguagePack
mk = RegisterRussianName or rus.RegisterRussianName

--if mk then
	--Расшариваем функцию для всех.
	--А надо ли? Ведь мод еще нужно переконвертировать в UTF-8.
	--_G.rawset(_G,"RegisterRussianName",mk)
--end


if not mk then --Без русификатора вообще нет смысла что-либо предпринимать.
	print("Mod Translation: WARNING! Function \"RegisterRussianName\" not found.")
	return
end
...
if not (in_game) then
	...
	print("Mod Translation: Not in game. Aborting...")
	return
end

print("Mod Translation: Initialization done!")


local new_phrases = {nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,nil,}
mods.RussianLanguagePack.mod_phrases = new_phrases
...
rus.pp = RegisterRussianPhrase
--mods.RussianLanguagePack.mod_pp = pp --Экспортируем API функцию.
--mods.RussianLanguagePack.mod_RegisterPhrase = pp
...
mods.RussianLanguagePack.mod_RenameAction = RenameAction --Экспортируем API функцию.
...
mods.RussianLanguagePack.mod_RegisterRecipes = RegisterRecipes
